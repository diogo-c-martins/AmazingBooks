...
import com.example.amazingbooksapp.utils.Constants
import com.example.amazingbooksapp.utils.Constants.Companion.QUERY_PAGE_SIZE
import com.example.amazingbooksapp.utils.Constants.Companion.SEARCH_BOOKS_TIME_DELAY
...
import kotlinx.android.synthetic.main.fragment_search_books.paginationProgressBar
...
class SearchBooksFragment : Fragment(R.layout.fragment_search_books) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        booksAdapter.setOnItemClickListener {
            ...
            findNavController().navigate(
                R.id.action_searchBooksFragment_to_ItemFragment,
                bundle
            )
        }
            ...
            job = MainScope().launch {
                ...
                editable?.let {
                    if (editable.toString().isNotEmpty()) {
                        viewModel.searchBooks(editable.toString())
                    }
                }
            }
        }

        viewModel.searchBooks.observe(viewLifecycleOwner, Observer { response ->
            when (response) {
                is Resource.Success -> {
                    hideProgressBar()
                    response.data?.let { bookResponse ->
                        ...
                        if(isLastPage){
                            rvSearchBooks.setPadding(0,0,0,0)
                        }
                    }
                }
                is Resource.Error -> {
                    ...
                    response.message?.let {
                        Log.e(Constants.TAG_BOOK, "An error occurred")
                    }
                }
                is Resource.Loading -> {
                    showProgressBar()
    ...
    private fun setupRecyclerView() {
        ...
        rvSearchBooks.apply {
            ...
            addOnScrollListener(this@SearchBooksFragment.scrollListener)
        }
    }
